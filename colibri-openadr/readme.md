...
| Number| Usage| Description|
| :-------------: |:-------------| :-----|
|1 | print if connector is registered| This indicates if the connector is registered on the colibri side. The connector is only successfully registerd if the core sends a 200 ok status message back.  
...
|25|deregister the VEN at the VTN party|This issues an oadrCancelPartyRegistration message. The VTN replies with an oadrCanceledPartyRegistration. This reply inidcates if the VTN deregistered the VEN successfully.|
...
This technology makes the parsing of a json formated Sparql query result easy. This result is sent to this connector after a query message by the colibri core.
...
This implementation is fully complied with the Colibri semantic core: Interface Version 1.2.1 . Nevertheless  there is some space for interpretation.  
