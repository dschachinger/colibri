...
public class OpenADRColibriBridge {
    ...
    // This object transforms the colibri and openADR replies for colibri messages.
    private ColibriToOpenADR colibriToOpenADR;
    // This object transforms the colibri and openADR replies for openADR messages.
    private OpenADRToColibri openADRToColibri;

    // For every service (key: service URL) this map stores a sorted list which contains all received events. They are ordered by their active time.
    private HashMap<String, SortedDateIntervalList<MsgInfo_OADRDistributeEvent.Event>> serviceSortedReceivedEvents;

    // For every eventID (=key) this map stores the associated service URL (=value)
    private HashMap<String, String> eventsTyp;

    // For every sparql query (=key) this map stores the list of all query results (=values)
     ...
     * Therefor a given openADR message is converted into a colibri understandable format.
     * @param info openADR info message
     */
    ...
    /**
     * This method sends a fixed query to the colibri side. It will add the given list to a map.
     * Afterwards the method handle_QUERY_RESULT in the class ColibriToOpenADR adds the received result to the list.
    ...
    /**
     * This method returns for a given service name and a given time interval all the events which fit to these constraints.
     * A proper event has its fully active time between the given interval.
    ...
    /**
     * For a given service name and a given eventID this method returns the index in the sorted list of this corresponding event.
     * @param serviceName colibri service URL for load events or price events
    ...
    /**
     * This method removes the event with the given event ID from the sorted list and returns the removes information.
     * @param eventID unique event identifier
     * @return
     */
